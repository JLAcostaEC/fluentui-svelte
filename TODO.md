...
Note: Slider may be a legitimate exception — arrow keys there change a *value*, not focus.
...
const state = {
  ...
  set selectedItems(v) { selectedItems = v; },
  get anchorIndex() { return anchorIndex; },
  set anchorIndex(v) { anchorIndex = v; }
};
```
...
- [ ] Also revisit `getFSContext` — it reads `getAllContexts().entries().pop()` and relies on the
      *most recently set* context matching a name prefix. That is fragile under nesting and worth
